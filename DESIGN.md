...
### Skip Whole Category
A full-width outlined button ("Nothing needed – skip whole category") sits at the top of every category that has stocked items, so staff don't have to scroll a 30-item Makro section ticking Skip on each row. Tapping it runs every row through the same path as the per-item Skip checkbox, clears any entered stock/quantities, marks the category complete and advances to the next one. If numbers were already entered it arms first (button turns Charred Terracotta ink with "Tap again to clear N entered items and skip", reverts after 4s), the same two-tap rule as other destructive actions. Once skipped, the button reads "Category skipped ✓ Tap to undo" and the header gets an italic Faded Soot "skipped" tag; undo un-skips every item and reopens the category. The state is derived, not stored: a category counts as skipped whenever every skippable item in it is skipped, so ticking each row by hand looks identical. Categories made only of no-par items (Bottles to Return) don't get the button.
